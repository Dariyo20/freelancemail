...
class AutomationWorker {
   ...
   * 25 emails per slot = 100/day max across all four segments.
   */
  async start() {
    try {
      this.isRunning = true;
      ...
      const meAfricaTask = cron.schedule('0 6 * * 0-4', async () => {
        ...
        try {
          await emailService.processQueue(25, ME_AFRICA);
        } catch (error) {
      ...
      const ukEuTask = cron.schedule('0 7 * * 1-4', async () => {
        ...
        try {
          await emailService.processQueue(25, UK_EU);
        } catch (error) {
      ...
      const usPrimaryTask = cron.schedule('0 14 * * 1-4', async () => {
        ...
        try {
          await emailService.processQueue(25, US_CA);
        } catch (error) {
